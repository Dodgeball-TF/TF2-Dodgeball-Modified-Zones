...
#define PLUGIN_NAME           "[TF2] Dodgeball Zones"
#define PLUGIN_AUTHOR         "Damizean, Tolfx, edited by x07x08"
...
public void OnPluginStart()
{
  ...
  g_hCvarEffectOnTarget         = CreateConVar("tf_dodgeball_effect_on_target", "0", "Glow effect when the rocket is targeting player (Only visible for the target no one else)", _, true, 0.0, true, 1.0);
...
void EnableDodgeBall()
{
  ...
  char strMapFile[PLATFORM_MAX_PATH];
  FormatEx(strMapFile, sizeof(strMapFile), "%s.cfg", strMapName);
...
void DisableDodgeBall()
{
  ...
  // Execute enable config file
  char strCfgFile[64];
  g_hCvarDisableCfgFile.GetString(strCfgFile, sizeof(strCfgFile));
  ServerCommand("exec \"%s\"", strCfgFile);
...
public void OnPlayerSpawn(Event hEvent, char[] strEventName, bool bDontBroadcast)
{
  ...
  // Handle hWeapon = PrepareItemHandle(21, iClient, TF2_GetPlayerClass(iClient));
  // CloseHandle(hWeapon);
}
...
{
  Handle hGameConf = LoadGameConfigFile("tf2.wear");
...
public void OnPlayerDeath(Event hEvent, char[] strEventName, bool bDontBroadcast)
{
  ...
  if (iIndex == -1)
  {
    // Find rocket and destroy it, depending if the rocket target was the victim that went bye bye for no reason than rocket
    for (int i = 0; i < MAX_ROCKETS; i++)
    {
      if (g_iRocketTarget[i] == EntIndexToEntRef(iVictim))
      {
        DestroyRocket(i);
  ...
  if (!IsValidClient(iTarget) || !isInsideZone(iTarget))
  {
  ...
  // Destroy any additional rockets targeting the victim
  int rocketCount = 0;
  for (int i = 0; i < MAX_ROCKETS; i++)
  {
    if (g_iRocketTarget[i] == EntIndexToEntRef(iVictim))
...
public void Respawn(int serial)
{
...
public void OnPlayerInventory(Event hEvent, char[] strEventName, bool bDontBroadcast)
{
  ...
  for (int iSlot = iClass == TFClass_Pyro ? 1 : 0; iSlot < 5; iSlot++)
  {
...
stock Handle PrepareItemHandle(int weaponLookupIndex, int client, TFClassType classbased = TFClass_Unknown)
{
  char weaponClassname[64] = "tf_weapon_flamethrower";
  int  weaponIndex         = 21;
  int  weaponSlot          = 0;
  int  weaponQuality       = 0;
  int  weaponLevel         = 1;

  int  flags               = OVERRIDE_CLASSNAME | OVERRIDE_ITEM_DEF | OVERRIDE_ITEM_LEVEL | OVERRIDE_ITEM_QUALITY | OVERRIDE_ATTRIBUTES;
  ...
  LogMessage("Preparing item handle for weapon index %d and class %d", weaponLookupIndex, classbased);
  LogMessage("Weapon handle prepared: %d", hWeapon);
  ...
  // int vm = CreateEntityByName("tf_wearable_vm");
  // SetEntProp(vm, Prop_Send, "m_nModelIndex", PrecacheModel("models/weapons/c_models/c_flamethrower/c_flamethrower.mdl"));
  // SetEntProp(vm, Prop_Send, "m_fEffects", EF_BONEMERGE | EF_BONEMERGE_FASTCULL);
  // SetEntProp(vm, Prop_Send, "m_iTeamNum", GetClientTeam(client));
  // SetEntProp(vm, Prop_Send, "m_usSolidFlags", 4);
  // SetEntProp(vm, Prop_Send, "m_CollisionGroup", 11);
  // DispatchSpawn(vm);
  // SetVariantString("!activator");
  // ActivateEntity(vm);
  // TF2_EquipWearable(client, vm);

  // SetEntPropEnt(vm, Prop_Send, "m_hWeaponAssociatedWith", entity);
  // SetEntPropEnt(entity, Prop_Send, "m_hExtraWearableViewModel", vm);
  // SetEntPropFloat(vm, Prop_Send, "m_flModelScale", 0.4);
...
public int AmountOfPlayersInsideZone()
{
  int iAmount = 0;
  for (int iClient = 1; iClient <= MaxClients; iClient++)
  {
    ...
    if (fuckZones_IsClientInZone(iClient, ZONE_NAME)) iAmount++;
  }
...
public Action OnDodgeBallGameFrame(Handle hTimer, any Data)
{
  ...
  // Calculate the maximum number of rockets based on the number of players inside the zones
  int maxRockets = 1 + (iAmount / 4);
...
void HomingRocketThink(int iIndex)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (!IsValidClient(iTarget, true) || !isInsideZone(iTarget))
      {
        int iOwner = iTarget;
...
void RocketOtherThink(int iIndex)
{
  ...
  {
    if (((GetGameTime() - g_fRocketLastDeflectionTime[iIndex]) >= g_fRocketClassControlDelay[iClass]) && (g_iRocketFlags[iIndex] & RocketFlag_Elevating))
    {
...
{
  return iDeflections + (g_iRocketsFired * g_fRocketClassRocketsModifier[iClass]) + (g_iPlayerCount * g_fRocketClassPlayerModifier[iClass]);
}
...
void ApplyRocketParameters(int iIndex)
{
  ...
  CopyVectors(g_fRocketDirection[iIndex], fVelocity);
...
public Action CmdShockwave(int iArgs)
{
  ...
  iDamage = StringToInt(strBuffer);
  GetCmdArg(3, strBuffer, sizeof(strBuffer));
  fPushStrength = StringToFloat(strBuffer);
  GetCmdArg(4, strBuffer, sizeof(strBuffer));
  fRadius = StringToFloat(strBuffer);
  GetCmdArg(5, strBuffer, sizeof(strBuffer));
  fFalloffRadius = StringToFloat(strBuffer);
...
void ExecuteCommands(DataPack hDataPack, int iClass, int iRocket, int iOwner, int iTarget, int iLastDead, float fSpeed, int iNumDeflections, float fMphSpeed)
{
  ...
  while (iNumCommands-- > 0)
  {
    ...
    FormatEx(strBuffer, sizeof(strBuffer), "%i", iRocket);
    ReplaceString(strCmd, sizeof(strCmd), "@rocket", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", iOwner);
    ReplaceString(strCmd, sizeof(strCmd), "@owner", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", iTarget);
    ReplaceString(strCmd, sizeof(strCmd), "@target", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", iLastDead);
    ReplaceString(strCmd, sizeof(strCmd), "@dead", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", iNumDeflections);
    ReplaceString(strCmd, sizeof(strCmd), "@deflections", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%f", fSpeed);
    ReplaceString(strCmd, sizeof(strCmd), "@speed", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", RoundToNearest(fMphSpeed));
    ReplaceString(strCmd, sizeof(strCmd), "@mphspeed", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%i", RoundToNearest(fSpeed * 0.042614));
    ReplaceString(strCmd, sizeof(strCmd), "@capmphspeed", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%f", fMphSpeed / 0.042614);
    ReplaceString(strCmd, sizeof(strCmd), "@nocapspeed", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%.2f", fSpeed);
    ReplaceString(strCmd, sizeof(strCmd), "@2dspeed", strBuffer);
    FormatEx(strBuffer, sizeof(strBuffer), "%.2f", fMphSpeed / 0.042614);
    ReplaceString(strCmd, sizeof(strCmd), "@2dnocapspeed", strBuffer);
...
void ParseClasses(KeyValues kvConfig)
{
  ...
  do
  {
    ...
    // Basic parameters
    kvConfig.GetSectionName(strName, sizeof(strName));
    strcopy(g_strRocketClassName[iIndex], 16, strName);
    kvConfig.GetString("name", strBuffer, sizeof(strBuffer));
    strcopy(g_strRocketClassLongName[iIndex], 32, strBuffer);
    ...
    {
      iFlags |= RocketFlag_OnSpawnCmd;
      g_hRocketClassCmdsOnSpawn[iIndex] = hCmds;
    }

    kvConfig.GetString("on deflect", strBuffer, sizeof(strBuffer));
    if ((hCmds = ParseCommands(strBuffer)) != null)
    {
      iFlags |= RocketFlag_OnDeflectCmd;
      g_hRocketClassCmdsOnDeflect[iIndex] = hCmds;
    }

    kvConfig.GetString("on kill", strBuffer, sizeof(strBuffer));
    if ((hCmds = ParseCommands(strBuffer)) != null)
    {
      iFlags |= RocketFlag_OnKillCmd;
      g_hRocketClassCmdsOnKill[iIndex] = hCmds;
    }

    kvConfig.GetString("on explode", strBuffer, sizeof(strBuffer));
    if ((hCmds = ParseCommands(strBuffer)) != null)
    {
      iFlags |= RocketFlag_OnExplodeCmd;
      g_hRocketClassCmdsOnExplode[iIndex] = hCmds;
    }

    kvConfig.GetString("on no target", strBuffer, sizeof(strBuffer));
    if ((hCmds = ParseCommands(strBuffer)) != null)
    {
      iFlags |= RocketFlag_OnNoTargetCmd;
      g_hRocketClassCmdsOnNoTarget[iIndex] = hCmds;
    }
...
stock bool isInsideZone(int client)
{
  return fuckZones_IsClientInZone(client, ZONE_NAME);
}
...
stock int SelectTarget(int iTeam, int iRocket = -1)
{
  ...
  for (int iClient = 1; iClient <= MaxClients; iClient++)
  {
    ...
    if (bUseRocket)
    {
      static float fClientPosition[3];
      GetClientEyePosition(iClient, fClientPosition);
      static float fDirectionToClient[3];
  ...
  if (g_hCvarEffectOnTarget.BoolValue && iRocket != -1)
  {
    if (g_iRocketParticle[iRocket] != -1)
    {
      SDKUnhook(g_iRocketParticle[iRocket], SDKHook_SetTransmit, HookEffectOnRocketTarget);
      RemoveEdict(g_iRocketParticle[iRocket]);
      g_iRocketParticle[iRocket] = -1;
    }
    int particle = SetEffectOnRocket(iRocket);
    if (particle != -1)
    {
      g_iRocketParticle[iRocket] = particle;
    }
  }
...
stock int SetEffectOnRocket(int rocketIndex)
{
  int glow         = CreateEntityByName("tf_glow");
  int rocketEntity = EntRefToEntIndex(g_iRocketEntity[rocketIndex]);

  if (IsValidEdict(glow) && IsValidEdict(rocketEntity))
  {
    char strName[126];
    Format(strName, sizeof(strName), "rocket_%i", rocketIndex);
    DispatchKeyValue(rocketEntity, "targetname", strName);

    DispatchKeyValue(glow, "targetname", "glow_rocket");
    DispatchKeyValue(glow, "target", strName);
    DispatchKeyValue(glow, "Mode", "0");
    DispatchSpawn(glow);
    AcceptEntityInput(glow, "Enable");

    SDKHook(glow, SDKHook_SetTransmit, HookEffectOnRocketTarget);
...
stock Action HookEffectOnRocketTarget(int glow, int other)
{
  int rocketIndex = FindRocketByEntity(GetEntPropEnt(glow, Prop_Send, "m_hTarget"));
  int target      = EntRefToEntIndex(g_iRocketTarget[rocketIndex]);
  if (target == other)
  {
    return Plugin_Continue;
  }
...
void CheckStolenRocket(int iClient, int iIndex)
{
  ...
  if (!(iTarget != iClient && !bStealArray[iClient].stoleRocket && (GetEntitiesDistance(iTarget, iClient) > g_hCvarStealDistance.FloatValue) && (!(g_iRocketFlags[iIndex] & RocketFlag_StealTeamCheck) || (GetClientTeam(iTarget) == GetClientTeam(iClient))) && !(g_iRocketState[iIndex] & RocketState_Delayed)))
  {
    return;
  }
...
public Action OnTouch(int iEntity, int iOther)
{
  ...
  static float vNewAnglesRef[3];
  CopyVectors(vNewAngles, vNewAnglesRef);
  static float vBounceVecRef[3];
...
public Action OnTFExplosion(const char[] strTEName, const int[] iClients, int iNumClients, float fDelay)
{
  ...
  TE_ReadVector("m_vecNormal", vecNormal);
